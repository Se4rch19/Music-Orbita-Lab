...
export type PlayVoiceOpts = {
  ...
  send?: AudioNode;
  sendGain?: number;
  /** v1.2 shorter harmonic tails. Leave false to hear v1.1 A/B. */
  cleanTails?: boolean;
};
...
function playPad(opts: PlayVoiceOpts): AudioNode[] {
  ...
  const releaseAt = time + Math.max(0.4, duration - (opts.cleanTails === false ? 0.35 : 0.1));
  mix.gain.setTargetAtTime(0.0001, releaseAt, opts.cleanTails === false ? 0.32 : 0.14);
  ...
  lfo.start(time);
  lfo.stop(time + duration + (opts.cleanTails === false ? 0.5 : 0.22));
  ...
  for (const layer of layers) {
    ...
    g.connect(mix);
    osc.start(time);
    osc.stop(time + duration + (opts.cleanTails === false ? 0.35 : 0.18));
    nodes.push(osc, g);
  }
  return nodes;
}
...
function playBass(opts: PlayVoiceOpts): AudioNode[] {
  ...
  thirdG.gain.value = opts.cleanTails === false ? 0.22 : 0.1;
  const gain = ctx.createGain();
...
function playBell(opts: PlayVoiceOpts): AudioNode[] {
  ...
  mix.gain.exponentialRampToValueAtTime(
    0.0001,
    time + Math.max(opts.cleanTails === false ? 0.45 : 0.28, duration * (opts.cleanTails === false ? 1.6 : 1.12)),
  );
  const output = panNode(ctx, dest, pan);
  mix.connect(output);
  ...
  ratios.forEach((ratio, i) => {
    ...
    partial.connect(mix);
    osc.start(time);
    osc.stop(time + duration + (opts.cleanTails === false ? 0.35 : 0.2));
...
function playKick(opts: PlayVoiceOpts): AudioNode[] {
  ...
  clickGain.gain.setValueAtTime(lvl * (opts.cleanTails === false ? 0.22 : 0.1), time);
  clickGain.gain.exponentialRampToValueAtTime(0.0001, time + (opts.cleanTails === false ? 0.022 : 0.012));
...
function playSnare(opts: PlayVoiceOpts): AudioNode[] {
  ...
  bp.Q.value = opts.cleanTails === false ? 0.65 : 0.5;
  const gain = ctx.createGain();
  ...
  body.frequency.exponentialRampToValueAtTime(opts.cleanTails === false ? 130 : 110, time + 0.06);
  const bodyGain = ctx.createGain();
  bodyGain.gain.setValueAtTime(lvl * (opts.cleanTails === false ? 0.42 : 0.18), time);
  bodyGain.gain.exponentialRampToValueAtTime(0.0001, time + (opts.cleanTails === false ? 0.12 : 0.07));
...
function playPickup(opts: PlayVoiceOpts): AudioNode[] {
  ...
  nGain.gain.setValueAtTime(lvl * (opts.cleanTails === false ? 0.7 : 0.32), time);
