...
export function composeBar(ctx: ComposeContext): BarScore {
  ...
  if (section === "TRANSITION") {
    ...
    addPad(notes, voicing, density, mode.space, section, ctx.intensity, world, chordChanged);
    addBass(notes, voicing, session, density, rng, section, mode.pulse, ctx.intensity, polish);
    ...
    addMelody(notes, voicing, session, ctx, section, density, rng, policy, polish);
    ...
    if (wantCounter && rng.chance(0.82)) {
      addCounter(notes, voicing, session, ctx, rng, policy, polish);
    }
  }
...
function addBass(
  ...
  if (section === "INTRO" && intensity < 0.28 && !driving) {
    notes.push({ tick: 0, durationTicks: 15, midi: root, velocity: 0.92, voice: "bass", priority: 7 });
    notes.push({ tick: 8, durationTicks: 6, midi: fifth, velocity: 0.72, voice: "bass", priority: 7 });
    return;
  }
...
function addPercussion(
  ...
  if (section === "RECOVERY") {
    notes.push({ tick: 0, durationTicks: 2, midi: 92, velocity: 0.5, voice: "click", priority: 2 });
    if (world === "glaciar" || world === "lavanda") {
      notes.push({ tick: 8, durationTicks: 2, midi: 91, velocity: 0.48, voice: "bell", priority: 3 });
    }
    if (world === "durazno" || world === "eclipse") {
      notes.push({ tick: 0, durationTicks: 3, midi: 36, velocity: 0.7, voice: "kick", priority: 6 });
      notes.push({ tick: 8, durationTicks: 2, midi: 48, velocity: 0.5, voice: "snare", priority: 4 });
    }
    return;
  }
...
function addCounter(
  ...
  notes.push(...realized.filter((n) => n.tick < TICKS_PER_BAR).map((n) => ({ ...n, velocity: n.velocity * 0.85, priority: 3 })));
}
