...
import type { EngineRevision, OrbitaThemeId, VoiceName } from "./types.ts";
...
export type Mixer = {
  ...
  setThemeMix: (theme: OrbitaThemeId, revision?: EngineRevision) => void;
  setMixProfile: (next: MixProfile) => void;
  setSpeakerPreview: (on: boolean) => void;
  prepareHarmonicChange: (time: number, seconds?: number) => void;
...
export function createMixer(ctx: BaseAudioContext): Mixer {
  ...
  const applyEq = () => {
    ...
    hipass.frequency.setTargetAtTime(profile.eq.hipass, t, 0.05);
    mud.gain.setTargetAtTime(profile.eq.mudGain, t, 0.05);
    if (!mixer.speakerPreview) {
      presence.gain.setTargetAtTime(profile.eq.presenceGain, t, 0.05);
      air.gain.setTargetAtTime(profile.eq.airGain, t, 0.05);
    }
  };
  ...
  const mixer: Mixer = {
    ...
    setSpeakerPreview(on) {
      ...
      air.gain.value = on ? 2.0 : profile.eq.airGain;
    },
    prepareHarmonicChange(time, seconds = 0.28) {
