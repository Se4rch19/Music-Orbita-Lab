...
export class OrbitaMusicEngine {
  ...
  private revision: EngineRevision = "v1.2";
  ...
  triggerLightPickup(options: LightPickupOptions = {}): void {
    ...
    if (this.combo >= 14 && rng.chance(0.55)) {
  ...
  getDiagnostics(): EngineDiagnostics {
    ...
    return {
      ...
      beatsPerBar: 4,
      subdivision: 4,
      section: this.currentScore()?.section ?? this.lastSection,
    ...
    for (let bar = 0; bar < bars; bar++) {
      ...
      for (const note of score.notes) {
        ...
        const nodes = playVoice(note.voice, {
          ...
          sendGain: mixer.sendAmount(note.voice),
  ...
  private scheduleAhead() {
    ...
    for (let bar = startBar; bar <= endBar; bar++) {
      ...
      const forceSection = this.transitioning && this.pending && bar < this.pending.switchAtBar ? "TRANSITION" : undefined;
