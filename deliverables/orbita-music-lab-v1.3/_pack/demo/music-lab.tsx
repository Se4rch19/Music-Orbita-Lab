...
export function MusicLab() {
  ...
  const [revision, setRevision] = useState<EngineRevision>("v1.3");
    ...
    <div className="min-h-dvh bg-bg text-fg">
      <div className="mx-auto flex max-w-6xl flex-col gap-5 px-4 py-6 pb-16 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-medium tracking-[0.22em] text-muted uppercase">Órbita · v1.3 composer pass</p>
            <h1 className="mt-1 font-display text-4xl font-medium tracking-tight text-fg sm:text-5xl">
              Music Lab
            </h1>
            <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted">
              Original procedural soundtrack. v1.3 writes phrases: layers share a plan, motifs repeat before they transform, and each world keeps a curated backbone.
            </p>
          </div>
        ...
        <section className="rounded-lg bg-surface p-4 shadow-border sm:p-5">
          ...
          <p className="mt-2 text-sm text-muted">
            ...
            Switch engine revision to hear v1.1 mix, v1.2 polish, and v1.3 composition on the same identity.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {(["v1.3", "v1.2", "v1.1"] as EngineRevision[]).map((id) => (
              <Button
                key={id}
                size="sm"
...
const SECTION_LOOP: SectionId[] = ["INTRO", "A", "B", "BUILD", "PEAK", "RECOVERY"];

function ComposerDebug({
  diag,
  onSolo,
  onMute,
  onClear,
  ...
  onSolo: (layer: LabLayer, on: boolean) => void;
  onMute: (layer: LabLayer, on: boolean) => void;
  onClear: () => void;
  onLoop: (section: SectionId | null) => void;
  onRestart: () => void;
  onNext: () => void;
}) {
  ...
  return (
    <section className="rounded-lg bg-surface p-4 shadow-border sm:p-5">
      <Label>Composer debug</Label>
      <p className="mt-2 text-sm text-muted">
        Lab only. Solo a layer to hear whether the counter, bass, or arp is the problem. Loop a section to judge phrasing. Not exposed in the game.
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        {SECTION_LOOP.map((id) => (
          <Button
        ...
        <table className="w-full min-w-[32rem] text-left text-xs">
          <thead>
            <tr className="text-muted">
              ...
              <th className="pb-2 font-medium">Solo</th>
            ...
            {LAB_LAYERS.map((layer) => {
              ...
              return (
                <tr key={layer} className="border-t border-border">
                  <td className="py-2 capitalize">{layer}</td>
                  <td className="py-2">
                    <button
                      type="button"
      ...
      {phrase ? (
        <dl className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
          <PlanStat label="Phrase" value={`#${phrase.phraseIndex} ${phrase.role}`} />
          <PlanStat label="Cadence" value={phrase.cadence} />
          <PlanStat label="Motif" value={phrase.motifVersion} />
          <PlanStat label="Contour" value={phrase.contour} />
          <PlanStat label="Bass" value={phrase.bassRole} />
          <PlanStat label="Tension" value={`${phrase.tensionOwner} · budget ${phrase.dissonanceBudget}`} />
          <PlanStat label="Center" value={phrase.tonalCenter} />
          <PlanStat label="Chords" value={phrase.chords.slice(0, 4).join(" → ")} />
          <PlanStat label="Layers" value={phrase.orchestration.join(", ")} />
        </dl>
      ) : (
        <p className="mt-4 text-xs text-muted">Play a world to inspect the current 8-bar phrase.</p>
      )}
    </section>
  );
}
...
function PlanStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-sm bg-bg px-3 py-2">
      ...
      <dd className="mt-1 font-mono text-[11px] leading-snug text-fg break-all">{value}</dd>
    </div>
  );
}
