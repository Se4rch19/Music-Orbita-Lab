...
import type { ChordVoicing, Motif, MotifStage, ScoreNote, VoiceName } from "../core/types.ts";
...
export function varyMotifComposed(motif: Motif, kind: MotifVariation): Motif {
  const contour = [...motif.contour];
  const rhythm = [...motif.rhythm];
  const rests = [...motif.rests];
  const accents = [...motif.accents];
  switch (kind) {
    case "transpose":
      return copy(motif, "t", contour.map((d) => d + 2), rhythm, rests, accents);
    case "invert": {
      const pivot = contour[0] ?? 0;
      return copy(motif, "i", contour.map((d) => pivot - (d - pivot)), rhythm, rests, accents);
    }
    ...
    case "displace":
      return copy(motif, "d", contour, [1, ...rhythm], [true, ...rests], [false, ...accents]);
    case "octave":
      return copy(motif, "o", contour.map((d, i) => (i % 2 === 0 ? d + 7 : d)), rhythm, rests, accents);
    case "fragment": {
      const cut = Math.max(2, Math.ceil(contour.length / 2));
      return copy(motif, "f", contour.slice(0, cut), rhythm.slice(0, cut), rests.slice(0, cut), accents.slice(0, cut));
    }
    case "extend":
      return copy(motif, "x", [...contour, 0], [...rhythm, 4], [...rests, false], [...accents, true]);
    case "response":
      return copy(motif, "r", contour.slice().reverse(), rhythm, rests, accents);
    default:
      return motif;
  }
}

export function variationForStage(stage: MotifStage, localBar: number): MotifVariation {
