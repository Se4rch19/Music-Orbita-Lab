...
describe("v1.3 composer grammar", () => {
  it("curates 3–5 progression families per campaign world", async () => {
    const { GRAMMARS } = await import("./grammar.ts");
    for (const id of ["menta", "durazno", "lavanda", "glaciar", "eclipse"] as const) {
      assert.ok(GRAMMARS[id].families.length >= 3 && GRAMMARS[id].families.length <= 5, id);
    }
  });

  it("plans 8-bar phrases with cadence, motif stage and tension owner", async () => {
    const { phrasePlanFor } = await import("./phrase.ts");
    const session = createSession({ themeId: "menta", seed: "MENTA-2026-001" });
    const plan = phrasePlanFor(session, 8, 0.5, 0);
    assert.equal(plan.section, "A");
    assert.equal(plan.length, 8);
    assert.ok(plan.bars.length === 8);
    assert.ok(["open", "half", "resolved", "deceptive", "suspended"].includes(plan.cadence));
    assert.ok(plan.motifId.length > 0);
    assert.equal(plan.bars[0]!.role, "statement");
    assert.equal(plan.bars[7]!.role, "cadence");
    assert.ok(plan.dissonanceBudget >= 0);
  });

  it("gives melody a rest inside an 8-bar Menta phrase", async () => {
    const { phrasePlanFor } = await import("./phrase.ts");
    const session = createSession({ themeId: "menta", seed: "MENTA-2026-001" });
    const plan = phrasePlanFor(session, 8, 0.5, 0);
    const silent = plan.bars.filter((b) => !b.melodyActive);
    assert.ok(silent.length >= 1, "phrase needs a breath");
    assert.ok(plan.bars[0]!.melodyActive, "statement must sing");
  });

  it("does not let melody and counter both run dense on the same bar", () => {
    const session = createSession({ themeId: "eclipse", seed: "E-ctp" });
    for (let i = 16; i < 24; i++) {
      const bar = composeBar({ session, barIndex: i, intensity: 0.7, combo: 4, mode: "expedition" });
      const melody = bar.notes.filter((n) => n.layer === "melody");
      const counter = bar.notes.filter((n) => n.layer === "counter");
      if (melody.length >= 3) {
        assert.ok(counter.length <= 2, `bar ${i} competing lines ${melody.length}/${counter.length}`);
      }
    }
  });

  it("keeps bass on chord tones at beats under the composer", () => {
    const session = createSession({ themeId: "durazno", seed: "D-bass-v13" });
    const bar = composeBar({ session, barIndex: 8, intensity: 0.55, combo: 2, mode: "expedition", composer: true });
    for (const note of bar.notes.filter((n) => n.voice === "bass")) {
      if (note.tick % 4 === 0) {
        assert.equal(isChordTone(note.midi, bar.chord), true, `bass ${note.midi} @${note.tick}`);
      }
    }
  });

  it("tags layers so lab mute/solo can isolate a voice", () => {
    const session = createSession({ themeId: "menta", seed: "MENTA-2026-001" });
    const bar = composeBar({ session, barIndex: 8, intensity: 0.5, combo: 0, mode: "expedition" });
    assert.ok(bar.notes.every((n) => n.layer));
    assert.ok(bar.notes.some((n) => n.layer === "harmony"));
    assert.ok(bar.notes.some((n) => n.layer === "bass"));
  });

  it("attaches a phrase diagnostic to the bar score", () => {
    const session = createSession({ themeId: "glaciar", seed: "G-plan" });
    const bar = composeBar({ session, barIndex: 6, intensity: 0.5, combo: 0, mode: "expedition" });
    assert.ok(bar.phrase);
    assert.equal(bar.phrase.section, bar.section);
    assert.ok(bar.phrase.chords.length >= 1);
    assert.ok(bar.phrase.motifVersion.length > 0);
  });

  it("exports grammar families and phrases on the plan JSON", () => {
    const session = createSession({ themeId: "lavanda", seed: "L-exp" });
    const json = exportPlan(session);
    assert.ok(json.grammarId);
    assert.ok(json.families && json.families.length >= 5);
    assert.ok(json.phrases && json.phrases.length >= 5);
  });

  it("v1.2 path still composes without a phrase diagnostic", () => {
    const session = createSession({ themeId: "menta", seed: "MENTA-2026-001" });
    const bar = composeBar({
      session,
      barIndex: 8,
    ...
    assert.equal(bar.phrase, undefined);
    assert.ok(bar.notes.some((n) => n.voice === "pad"));
    assert.ok(bar.notes.some((n) => n.voice === "bass"));
  });
});
