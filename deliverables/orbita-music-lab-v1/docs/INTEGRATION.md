...
- may add a short second tone at high combo
...
```ts
music.setMusicVolume(0.8); // 0..1
music.setSfxVolume(0.8);
music.setMuted(false, false); // music, events
```

Pickup / orbit / damage use the SFX bus so a player can lower music and still hear events.
